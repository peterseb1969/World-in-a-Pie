...
```json
{
  "mcpServers": {
    "wip": {
      ...
      "env": {
        "WIP_API_KEY_FILE": "/path/to/WorldInPie/data/secrets/api_key",
...
Apps created with the helper script in production mode use `WIP_API_KEY_FILE` in `.mcp.json`, which points to the shared key file. API key rotation in WIP automatically propagates to all apps — no per-app changes needed.

For apps created manually or in dev mode (hardcoded `WIP_API_KEY`), switching to prod requires:
1. Replace `WIP_API_KEY` with `WIP_API_KEY_FILE` in `.mcp.json`, pointing to `data/secrets/api_key`
