...
| **200 (per-item error)** | BulkResponse | Check each item's `status` field: `"created"`, `"updated"`, `"unchanged"`, `"skipped"`, `"error"` (on conflict, `status` is `"error"` with an `error_code` such as `already_exists`) | WIP write endpoints always return HTTP 200 with per-item results. Parse `results[]` and show errors inline per item.       |
