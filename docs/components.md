...
| POST | `/api/registry/entries/lookup/by-id` | Lookup by ID with 2-step cascade |
...
`POST /api/registry/entries/lookup/by-id` performs a **2-step resolution cascade**:

1. **Step 1 — `entry_id`:** Direct match on the canonical entry ID.
2. **Step 2 — `search_values`:** Match against the flat `search_values` array, which contains synonym values, merged/deprecated IDs, external IDs, and business keys from composite key fields.

The `namespace` and `entity_type` parameters are **optional**. Omit them to search across all namespaces/types; provide them to constrain the search.

The response includes a `matched_via` field indicating how the match was found (`entry_id` or `composite_key_value`). This enables the Document Store to resolve any identifier — canonical ID, synonym, or external ID — in a single call.
