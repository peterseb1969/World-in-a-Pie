...
3. **Mirror to kb (warn-and-continue)** — `bash ~/.cache/wip-kb-client/kb-client.sh add-to-kb.py "/Users/peter/Development/FR-YAC/reports/<SESSION-ID>/session.md"`. Composes `data.body` per the SESSION dispatch and POSTs `status=closed` + `ended_at`. If kb is unreachable, log to stderr and proceed — the local write is authoritative; the mirror retries at the next mirror-emitting action or via a manual `add-to-kb.py` re-run.
