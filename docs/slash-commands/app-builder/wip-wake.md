...
2. **Close the prior session** (load-bearing). All report paths below are under `/Users/peter/Development/FR-YAC/`:
   ...
   - Mirror the now-closed prior to kb: `bash ~/.cache/wip-kb-client/kb-client.sh add-to-kb.py "/Users/peter/Development/FR-YAC/reports/<prior-id>/session.md"`. **kb-unreachable → warn-and-continue** — the local write already flipped `status: closed`, so a later re-run sees `closed` and skips; the manual retry is the same `add-to-kb.py` command.
...
6. **Mirror the new session to kb (warn-and-continue)** — `bash ~/.cache/wip-kb-client/kb-client.sh add-to-kb.py "/Users/peter/Development/FR-YAC/reports/$NEW_ID/session.md"`. The loader derives the `CONTINUES_FROM` edge (new → prior) from the `continues_from:` frontmatter; if the prior isn't in kb yet, the edge silently skips and lands on a re-run. Both kb writes (step 2 close + step 6 create) are idempotent — re-running `/wip-wake` after a partial failure converges.
