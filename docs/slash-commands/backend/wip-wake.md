...
2. **Close the prior session** (load-bearing). Report paths below are **project-local** to this repo — `reports/<prior-id>/`, never the shared FR-YAC checkout:
   ...
   - Mirror the now-closed prior to kb (tier 3 only — skip silently if `.claude/kb.json` is absent): `bash ~/.cache/wip-kb-client/kb-client.sh kb-write.py SESSION reports/<prior-id>/session.md`. **kb-unreachable → warn-and-continue** — the local write already flipped `status: closed`, so a later re-run sees `closed` and skips; the manual retry is re-running the same `kb-write.py` call.
