...
export const useTermStore = defineStore('term', () => {
  ...
  async function fetchTerms(termId: string, params?: {
    ...
    error.value = null
    try {
      const response = await apiClient.listTerms(termId, params)
  ...
  async function fetchTerm(termId: string) {
    ...
    error.value = null
    try {
      currentTerm.value = await apiClient.getTerm(termId)
  ...
  async function createTerm(termologyId: string, data: CreateTermRequest) {
    ...
    error.value = null
    try {
      const created = await apiClient.createTerm(termologyId, data)
  ...
  async function updateTerm(termId: string, data: UpdateTermRequest) {
    ...
    error.value = null
    try {
      const updated = await apiClient.updateTerm(termId, data)
  ...
  async function deprecateTerm(termId: string, data: DeprecateTermRequest) {
    ...
    error.value = null
    try {
      const updated = await apiClient.deprecateTerm(termId, data)
      const index = terms.value.findIndex(t => t.term_id === termId)
      if (index !== -1) {
        terms.value[index] = updated
      }
  ...
  async function deleteTerm(termId: string) {
    ...
    error.value = null
    try {
      await apiClient.deleteTerm(termId)
  ...
  async function bulkCreateTerms(
    ...
    error.value = null
    try {
      const result = await apiClient.bulkCreateTerms(termologyId, data)
