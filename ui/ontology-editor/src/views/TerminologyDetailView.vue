...
import TerminologyForm from '@/components/TerminologyForm.vue'
import BulkTermImport from '@/components/BulkTermImport.vue'
...
async function exportTerminology(format: 'json' | 'csv') {
  ...
  try {
    const data = await apiClient.exportTerminology(props.id, format)
