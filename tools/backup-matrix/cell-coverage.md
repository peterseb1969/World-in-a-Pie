...
- **B-03 (BUILT, gated; both halves live-validated):** see §5.1. Latest green run: dev-test 20260726b (4/4, slice-8 board) — the near-empty second install is the natural place to run it, with no tenant data in scope and no instance-sized archive.
...
Slice 6 **BUILT + green on prod-test 20260726b** (23 cells, 22 pass, 0 fail,
1 skip; F cells gated behind the new `--allow-perturb` flag, which requires a
k8s `--install` and a kubectl context — like B-03 they change the install's
runtime state, so they are opt-in and reported as SKIP without the flag):
- **R-04 (BUILT):** the CLI-export → engine-restore seam live — and the cell's
  first run caught the wip-toolkit `check_health` proxy-mode defect (root
  `/health` unroutable through the ingress; fixed to the prefixed route).
- **F-06 (BUILT, gated):** reporting-sync down → restore warns-and-completes →
  visible parity gap → force backfill converges parity AND FTS.
- **F-05 (BUILT, gated):** force-kill mid-restore with outrace retry;
  phase-aware visibility assertions; blind re-run refused; drop-and-rerun
  recovery converges. Two cell-design lessons cost three red runs: a graceful
  delete lets the run FINISH inside the grace window, and an is-it-back probe
  can pass against the draining old pod and strand later cells on 503s —
  hence force-kill + wait-for-replacement-pod.

Slice 7 **BUILT + green: R-03 cross-instance DR** (24 cells, 23 pass, 0 fail,
1 skip on prod-test 20260726b → dev-test 20260726b; the dev-test install was
brought up for exactly this — `wip-deploy install --target k8s --preset
standard --namespace dev-test --hostname dev-test.internal`). Also hardened
F-06 against post-scale-up transport blips (a keep-alive reset in the window
where the ingress endpoints repopulate is a retry, not a finding).

Slice 8 **BUILT + the first fully complete board — 24 cells, 24 pass, 0 fail,
0 skip** (primary dev-test 20260726b, DR target prod-test 20260726b, both
opt-in gates open; both instances verified clean after). The last two sweep
generalizations landed inside the cells that own the right target states:
R-01 gained the id-preserving dry-run parity step, R-08 gained the merge
dry-run parity step and the all-unchanged re-run (X-05's merge variant), and
B-03 got its green run on the near-empty second install. F-06's kubectl
scale calls got checked return codes + retries (`_scale_deployment`) after
one spurious red: a transient scale failure previously read as a finding,
