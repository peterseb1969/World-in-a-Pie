...
class TestNamespaceAPI:
    """Tests for namespace management API."""

    @pytest.mark.asyncio
    async def test_create_namespace(self, client: AsyncClient, auth_headers: dict):
        ...
        assert data[0]["namespace_id"] == "test-namespace"
    ...
    async def test_list_namespaces(self, client: AsyncClient, auth_headers: dict):
        ...
        await client.post(
            "/api/registry/namespaces",
            json=[{"namespace_id": "list-test", "name": "List Test"}],
            headers=auth_headers
        )
...
class TestRegistrationAPI:
    ...
    async def test_register_duplicate_key(self, client: AsyncClient, auth_headers: dict):
        ...
        response1 = await client.post(
            "/api/registry/entries/register",
            json=[{"namespace": "default", "composite_key": key}],
            headers=auth_headers
        )
