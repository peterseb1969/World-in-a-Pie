"""The router preserves the /api/document-store prefix, so the root payload (with its
build-provenance block) must also be served at the prefixed path — otherwise
router-side consumers (the console's build probe at /api/document-store/) get a 404 and
...
async def test_prefixed_root_serves_build_provenance(client: AsyncClient):
    response = await client.get("/api/document-store/")
    assert response.status_code == 200
    data = response.json()
    assert data["service"] == "WIP Document Store"
    build = data["build"]
