#!/usr/bin/env python3
"""
WIP Dev Delete — Hard-delete entities from MongoDB by WIP ID.

DEVELOPMENT ONLY. Bypasses soft-delete, removes all versions, and cleans
up Registry entries so IDs can be re-used.
...
def delete_entity(
    ...
    if cascade and etype in CASCADE_RULES:
        for rule in CASCADE_RULES[etype]:
            # Get the match value from the parent entity
    ...
    for rule, match_val, _ in cascade_deletes:
        ...
        if child_id_field:
            ...
            # Clean up registry entries for children
            if child_ids:
...
def main():
    parser = argparse.ArgumentParser(
        description="Hard-delete WIP entities from MongoDB (dev only)",
        ...
        if not matches:
            ...
            if reg_count:
                ...
                if args.force:
                    ...
                    print(f"  Cleaned orphan registry entries")
            continue

        for etype, info, count in matches:
            print(f"\n  Type: {etype}")
            delete_entity(client, wip_id, etype, info, args.cascade, args.force)
