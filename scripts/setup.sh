...
load_config() {
    ...
    # WIP_DATA_DIR is already the canonical variable (set at top of script from env)
...
WIP_FILE_STORAGE_PUBLIC_ENDPOINT=http://${HOSTNAME:-localhost}:9000
...
generate_service_overrides() {
    log_step "Generating docker-compose override files..."

    local override_header
    override_header="# Auto-generated by setup.sh — DO NOT EDIT
...
# This file is .gitignored. Regenerate with: ./scripts/setup.sh --config config/last-install.conf -y"
    ...
    if [ "$VARIANT" = "dev" ]; then
        ...
        for svc_dir in registry def-store template-store document-store reporting-sync ingest-gateway; do
            ...
            # docker-compose service name (matches key in docker-compose.yml services:)
            local dc_svc
            case "$svc_dir" in
                def-store)       dc_svc="def-store" ;;
                template-store)  dc_svc="template-store" ;;
                document-store)  dc_svc="document-store" ;;
                reporting-sync)  dc_svc="reporting-sync" ;;
                ingest-gateway)  dc_svc="ingest-gateway" ;;
                *)               dc_svc="$svc_dir" ;;
            esac

            # Build override content — mount src and tests
            local override_file="$comp_dir/docker-compose.override.yml"
            local volumes="      - ./src:/app/src:ro
      - ./tests:/app/tests:ro"
...
EOF
        ...
        # Prod variant: remove any override files
        for svc_dir in registry def-store template-store document-store reporting-sync ingest-gateway; do
...
clean_nats_data() {
    ...
    podman stop wip-ingest-gateway wip-reporting-sync wip-nats 2>/dev/null || true
...
start_services() {
    ...
    if has_module "oidc"; then
        ...
        local grant_result
        grant_result=$(curl -s -X POST http://localhost:8001/api/registry/namespaces/wip/grants \
            -H "X-API-Key: $API_KEY" \
