...
- **Template `usage` annotation.** Templates carry `usage: "entity" | "reference" | "relationship"` (default `entity`). `relationship` enables document-to-document edges with two mandatory reference fields (`source_ref`, `target_ref`) and template-level `source_templates` / `target_templates` lists. Document writes against relationship templates run extra validation (cross-namespace and archived-endpoint rejected with `cross_namespace_relationship` / `archived_relationship_endpoint`). Two query endpoints become available: `GET /api/document-store/documents/{id}/relationships` and `…/traverse?depth=N` (depth capped at 10). `usage` is immutable after create. See `docs/design/document-relationships.md`.
