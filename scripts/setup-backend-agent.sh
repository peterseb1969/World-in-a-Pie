...
**Case discipline:**
- **Every case write is ONE gateway call** (CASE-464) — `POST <kb_app_url>/apps/kb/server-api/kb/cases[…]` per the served playbook (`~/.cache/wip-kb-client/case-workflow.md`). The server owns allocation (atomic `CASE-<n>` Registry-synonym claim — concurrent filers get distinct numbers by construction), the status machine, REFERENCES edges, and race-safe section appends. Never `Write` a case file with a hand-picked number; never reason about "the next number". (Lineage — each step retired its predecessor: FS `case-helper.sh claim` → served `case_allocate.py` → gateway verbs; the CASE-67/301 collisions started the discipline, CASE-306/425/437/464 name the steps.)
- **Flat files in `yac-discussions/` are optional write-staging.** Status lives in kb (`data.status`), maintained by the gateway verbs; a kept flat file's status-rename is FS-browsing courtesy, not a required step. There is no mirror step — the loaders that did it were retired by CASE-464 and refuse with a pointer.
