...
**Case discipline:**
- **Every case write is ONE gateway call** — `POST <kb_app_url>/apps/kb/server-api/kb/cases[…]` per the served playbook (`~/.cache/wip-kb-client/case-workflow.md`). The server owns allocation (atomic `CASE-<n>` Registry-synonym claim — concurrent filers get distinct numbers by construction), the status machine, REFERENCES edges, and race-safe section appends. Never `Write` a case file with a hand-picked number; never reason about "the next number".
- **Flat files in `yac-discussions/` are optional write-staging.** Status lives in kb (`data.status`), maintained by the gateway verbs; a kept flat file's status-rename is FS-browsing courtesy, not a required step. There is no mirror step — the loaders that did it were retired and refuse with a pointer.
