...
mkdir -p "$APP_DIR/.claude/scripts"
cp "$WIP_ROOT/agent-scripts/src/wake_rollover.py" "$APP_DIR/.claude/scripts/wake-rollover.py"
...
# Content lives in scaffold/templates/settings/app.json (CASE-612 step 2)
cp "$WIP_ROOT/scaffold/templates/settings/app.json" "$APP_DIR/.claude/settings.json"
...
# Content lives in scaffold/templates/hooks/post-compact-reanchor.sh (CASE-612 step 2)
cp "$WIP_ROOT/scaffold/templates/hooks/post-compact-reanchor.sh" "$APP_DIR/.claude/hooks/post-compact-reanchor.sh"
...
# Content lives in scaffold/templates/claude-md/app.md (CASE-612 step 2).
# Render = literal substitution of the four __TOKENS__ — python3, not sed:
# values (names, key-file paths) may contain sed metacharacters; replace()
# is exact, matching the old unquoted-heredoc interpolation byte-for-byte.
python3 - "$WIP_ROOT/scaffold/templates/claude-md/app.md" "$CLAUDE_TARGET" \
    "$APP_NAME" "$APP_SLUG" "$DEV_NAMESPACE" "$WIP_API_KEY_FILE" << 'PYRENDER'
import sys
src, dst, name, slug, ns, keyfile = sys.argv[1:7]
text = open(src).read()
for tok, val in (("__APP_NAME__", name), ("__APP_SLUG__", slug),
                 ("__DEV_NAMESPACE__", ns), ("__WIP_API_KEY_FILE__", keyfile)):
